...
class paging
{
	...
	function paging($baris=5, $langkah=5, $prev="Previous", $next="Next", $number="%%number%%")
	{
		$this->next=$next;
		$this->prev=$prev;
		$this->number=$number;
...
function number($i,$number)
	...
	function print_link()
	{
		...
		if ($this->p["count"]>$this->p["baris"]) {
			...
			// print prev
			if ($this->page>1)
			$print_link .= "<div class='pagenumbers'><a href=\"".$_SERVER["PHP_SELF"]."?".$_SERVER["QUERY_STRING"]."&page=1\">First Page</a>&nbsp;<a href=\"".$_SERVER["PHP_SELF"]."?".$_SERVER["QUERY_STRING"]."&page=".($this->page-1)."\">".$this->prev."</a>\n";
			...
			if($this->page==1)
			{
			$print_link .="<div class='pagenumbers'>";
			}
			// print active
			...
			$print_link .= "<a href=\"".$_SERVER["PHP_SELF"]."?".$_SERVER["QUERY_STRING"]."&page=".($this->page+1)."\">".$this->next."</a>&nbsp;<a href=\"".$_SERVER["PHP_SELF"]."?".$_SERVER["QUERY_STRING"]."&page=".$this->p["total_page"]."\">Last Page</a></div>\n";
