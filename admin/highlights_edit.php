...
if($_POST['click']=="2"){
	...
	$select=$db->query("select * from projects where title='".mysql_real_escape_string($_POST['title'])."'");
   ...
   if($image) {
   $chg=$db->query("update projects set title='".mysql_real_escape_string($_POST['title'])."', banner= '$image' where id=".$_GET['id']);
   }
   else
   {
 $chg=$db->query("update projects set title='".mysql_real_escape_string($_POST['title'])."' where id=".$_GET['id']);
  }
    if($chg){
...
if($_GET['id']!=''){
$val = $db->getRow('SELECT * FROM projects WHERE id= ?', array($_GET['id']));
}else{
		...
		<div id="box" class="test">
			<form class="cmxform" id="form2" action="" method="post" enctype="multipart/form-data" name="form1">
				...
				<tr style="vertical-align:top">
				  <td align="left">Title<span style="color:#FF0000"> *</span></td><td align="left" width="2">:</td><td align="left"><input id="title" name="title" value="<?php echo $val['title'];?>" size="25"/></td></tr>
				...
				<tr style="vertical-align:top">
					...
					<td align="left" width="2">:</td><td align="left"><div id="filetypeid">
						<img src="../highlights/thumb/<?php echo $val['banner'];?>" />
					</div>
					</td>
				</tr>
