...
if ($_POST["keyword"] == "del"){
	$id=$_POST['del_id'];
	$select=$db->query("select * from ".CITY." where id=$id");
    if(mysql_num_rows($select) > 0)
	{
     $delete=$db->query("Delete from ".CITY." where id=$id");
		if($delete==TRUE)
		{
  ...
  $id=$_POST['del_id']; 
  $update = $db->query("update ".CITY." set status=0 where id=$id");
  $_SESSION['suss'] = "Status changed successfully ..";
 
 }else if($_POST["keyword"] == "dact"){
  $id=$_POST['del_id'];
  $update = $db->query("update ".CITY." set status=1 where id=$id");
  $_SESSION['suss'] = "Status changed successfully .."; 
 }
	       ...
	       $action=$_POST['act'];
				...
				{	
					while (list ($key,$val) = each ($box)) 
					{
					   if($action=="delete")
					   {
					    $delete =$db->query("Delete from ".CITY." where id=$val");
					    $_SESSION['suss'] = "Successfully deleted..";
					   }
						 if($action=="activate")
					   {
					    $update = $db->query("update ".CITY." set status=1 where id=$val");
					    $_SESSION['suss'] = "Status changed successfully ..";
					   }
						 if($action=="deactivevate")
					   {
					   $update = $db->query("update ".CITY." set status=0 where id=$val");
					   $_SESSION['suss'] = "Status changed successfully ..";
					   } 
					}
			   }
  ...
  <tr>
    <td align="center" valign="top" class="bg"><table width="970" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
	    ...
	    $sql_select="select * from ".CITY." ".$sql." order by id desc";
    ...
    <thead>
    	<tr>
           ...
           <th width="300px"><a href="javascript:void(0)">City</a></th>
	...
	?>        
    	<tr>
        	<td><input type="checkbox" name="chk[]" value="<?php echo $value['id']; ?>" class="noborder chk" /></td>
			 <td><?php echo $m; ?></td>
            <td><?php echo ucfirst($value['citys']); ?></td>
            <td><?php if($value['status']==1){ ?><a href="javascript: del(<?php echo $value['id']; ?>,'act');" title="Change status to inactive" class="ask vtip"><img src="images/active.jpg" alt="Status" title="" border="0" /></a><?php }else{ ?><a href="javascript: del(<?php echo $value['id']; ?>,'dact');" title="Change status to active" class="ask vtip"><img src="images/inactive.jpg" alt="Status" title="" border="0" /></a><?php } ?>&nbsp;&nbsp;<a href="country_edit.php?id=<?php echo $value['id']; ?>" title="Edit Careers" class="vtip"><img src="images/user_edit.png" alt="Edit" title="" border="0" /></a>&nbsp;&nbsp;<a href="javascript: del(<?php echo $value['id']; ?>,'del');" class="ask vtip" title="Delete Careers"><img src="images/trash.png" alt="Delete" title="" border="0" /></a></td>
        </tr>      
        <?php $m++; } ?>
		 <tfoot>
    	<tr>
	 ...
	 <tr><td colspan="6"><div class="pagination"><?php echo $paging->print_link();?></div></td></tr>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
		<tr>
		  <td align="center" class="errortext"><strong>No Careers Found.</strong></td>
		</tr>
		</table>
		<?php } ?>
		</form>
