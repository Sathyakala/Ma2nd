...
if ($_POST["keyword"] == "del"){
	$id=$_POST['del_id'];
	$select=$db->query("select * from ".USER." where id=$id");
  ...
  $id=$_POST['del_id']; 
  $update = $db->query("update ".USER." set status=0 where id=$id");
  $_SESSION['suss'] = "Status changed successfully ..";
 
 }else if($_POST["keyword"] == "dact"){
  $id=$_POST['del_id'];
  $update = $db->query("update ".USER." set status=1 where id=$id");
  $_SESSION['suss'] = "Status changed successfully .."; 
 }
	       ...
	       $action=$_POST['act'];
				...
				{	
					while (list ($key,$val) = each ($box)) 
					{
					   if($action=="delete")
					   {
					    $delete =$db->query("Delete from ".USER." where id=$val");
					    $_SESSION['suss'] = "Successfully deleted..";
					   }
						 if($action=="activate")
					   {
					    $update = $db->query("update ".USER." set status=1 where id=$val");
					    $_SESSION['suss'] = "Status changed successfully ..";
					   }
						 if($action=="deactivevate")
					   {
					   $update = $db->query("update ".USER." set status=0 where id=$val");
					   $_SESSION['suss'] = "Status changed successfully ..";
					   } 
					}
			   }
  ...
  <tr>
    <td align="center" valign="top" class="bg"><table width="970" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
	    ...
	    $sql_select="select * from ".USER." ".$sql." order by id desc";
    ...
    <thead>
    	<tr>
           ...
           <th width="350px"><a href="javascript:void(0)">First Name</a></th>
           <!--<th width="300px"><a href="javascript:void(0)">Last Name</a></th>-->
           <th width="300px"><a href="javascript:void(0)">Phone</a></th>		  
           <th width="300px"><a href="javascript:void(0)">Mobile</a></th>		   
           <th width="300px"><a href="javascript:void(0)">Email</a></th>		   
           <th width="300px"><a href="javascript:void(0)">Address 1</a></th>		   
          <!-- <th width="250px"><a href="javascript:void(0)">Address 2</a></th>-->
	...
	?>        
    	<tr>
            ...
            <td><a href="userpreview.php?id=<?php echo $value['id']; ?>" title="View News Detail" rel="facebox" class="vtip"><img src="images/viewicon.gif" alt="View Detail" title="" border="0" /></a>&nbsp;&nbsp;<?php if($value['status']==1){ ?><a href="javascript: del(<?php echo $value['id']; ?>,'act');" title="Change status to inactive" class="ask vtip"><img src="images/active.jpg" alt="Status" title="" border="0" /></a><?php }else{ ?><a href="javascript: del(<?php echo $value['id']; ?>,'dact');" title="Change status to active" class="ask vtip"><img src="images/inactive.jpg" alt="Status" title="" border="0" /></a><?php } ?>&nbsp;&nbsp;<a href="user_edit.php?id=<?php echo $value['id']; ?>" title="Edit News" class="vtip"><img src="images/user_edit.png" alt="Edit" title="" border="0" /></a>&nbsp;&nbsp;<a href="javascript: del(<?php echo $value['id']; ?>,'del');" class="ask vtip" title="Delete news"><img src="images/trash.png" alt="Delete" title="" border="0" /></a></td>
        </tr>      
        <?php $m++; } ?>
		 <tfoot>
    	<tr>
