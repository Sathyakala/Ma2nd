...
if ($_POST["keyword"] == "del"){
	$id=$_POST['del_id'];
	$select=$db->query("select * from ".PLOTS." where id=$id");
    if(mysql_num_rows($select) > 0)
	{
     $delete=$db->query("Delete from ".PLOTS." where id=$id");
		if($delete==TRUE)
		{
  ...
  $id=$_POST['del_id']; 
  $update = $db->query("update ".PLOTS." set status=0 where id=$id");
  $_SESSION['suss'] = "Status changed successfully ..";
 
 }else if($_POST["keyword"] == "dact"){
  $id=$_POST['del_id'];
  $update = $db->query("update ".PLOTS." set status=1 where id=$id");
  $_SESSION['suss'] = "Status changed successfully .."; 
 }
	       ...
	       $action=$_POST['act'];
				...
				{	
					while (list ($key,$val) = each ($box)) 
					{
					   if($action=="delete")
					   {
					    $delete =$db->query("Delete from ".PLOTS." where id=$val");
					    $_SESSION['suss'] = "Successfully deleted..";
					   }
						 if($action=="activate")
					   {
					    $update = $db->query("update ".PLOTS." set status=1 where id=$val");
					    $_SESSION['suss'] = "Status changed successfully ..";
					   }
						 if($action=="deactivevate")
					   {
					   $update = $db->query("update ".PLOTS." set status=0 where id=$val");
					   $_SESSION['suss'] = "Status changed successfully ..";
					   } 
					}
			   }
  ...
  <tr>
    <td align="center" valign="top" class="bg"><table width="970" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
	    ...
	    $sql_select="select * from ".PLOTS." ".$sql." order by id desc";
    ...
    <thead>
    	<tr>
           ...
           <th width="20px"><a href="javascript:void(0)">Plot No<img src="images/arrow_dows_mini.gif" width="16" height="16" align="absmiddle" /></a></th>
             <th width="300px"><a href="javascript:void(0)">Actual Dimesion</a></th>		  
           <th width="300px"><a href="javascript:void(0)">Corrected Dimension</a></th>		   
            <th width="300px"><a href="javascript:void(0)">Area</a></th>		   
           <th width="250px"><a href="javascript:void(0)">Facing</a></th>
		   <th width="250px"><a href="javascript:void(0)">Status</a></th>
	...
	?>        
    	<tr>
        	<td><input type="checkbox" name="chk[]" value="<?php echo $value['id']; ?>" class="noborder chk" /></td>
			 <td><?php echo ucfirst($value['plot_no']); ?></td>
             <td><?php echo ucfirst($value['actual_dimesion']); ?></td>			
            <td><?php echo ucfirst($value['corrected_dimension']); ?></td>			
             <td><?php echo ucfirst($value['area']); ?></td>			
            <td><?php echo ucfirst($value['facing']); ?></td>
            <td>
			<?php echo ucfirst($value['status']); ?>
			<!--<select name="status"  onChange="getData(this.value)">
				<option selected="selected" value="">Select Status</option>
				<option value="OPEN" <?php //if($value['status']=="OPEN") { echo "selected"; }?>>Open</option>
				<option value="BOOKED" <?php //if($value['status']=="BOOKED") { echo "selected"; }?>>Booked</option>
				<option value="BLOCKED" <?php //if($value['status']=="BLOCKED") { echo "selected"; }?>>Blocked</option>
			</select>-->
			<?php //echo ucfirst($value['status']); ?></td>

            <td><a href="edit_plot.php?id=<?php echo $value['id'];?>" title="Edit Plot List" class="vtip"><img src="images/user_edit.png" alt="Edit" title="" border="0" /></a>&nbsp;&nbsp;
				<a href="javascript: del(<?php echo $value['id']; ?>,'del');" class="ask vtip" title="Delete news"><img src="images/trash.png" alt="Delete" title="" border="0" /></a></td>
        </tr>      
        <?php $m++; } ?>
		 <tfoot>
    	<tr>
	 ...
	 <tr><td colspan="12"><div class="pagination"><?php echo $paging->print_link();?></div></td></tr>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
		<tr>
		  <td align="center" class="errortext"><strong>No Plot Details Found.</strong></td>
		</tr>
		</table>
		<?php } ?>
		</form>
