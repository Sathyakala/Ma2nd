...
if ($_POST["keyword"] == "del"){
	$id=$_POST['del_id'];
	$select=$db->query("select * from ".COUNTRY." where id=$id");
    if(mysql_num_rows($select) > 0)
	{
     $delete=$db->query("Delete from ".COUNTRY." where id=$id");
		if($delete==TRUE)
		{
  ...
  $id=$_POST['del_id']; 
  $update = $db->query("update ".COUNTRY." set status=0 where id=$id");
  $_SESSION['suss'] = "Status changed successfully ..";
 
 }else if($_POST["keyword"] == "dact"){
  $id=$_POST['del_id'];
  $update = $db->query("update ".COUNTRY." set status=1 where id=$id");
  $_SESSION['suss'] = "Status changed successfully .."; 
 }
	       ...
	       $action=$_POST['act'];
				...
				{	
					while (list ($key,$val) = each ($box)) 
					{
					   if($action=="delete")
					   {
					    $delete =$db->query("Delete from ".COUNTRY." where id=$val");
					    $_SESSION['suss'] = "Successfully deleted..";
					   }
						 if($action=="activate")
					   {
					    $update = $db->query("update ".COUNTRY." set status=1 where id=$val");
					    $_SESSION['suss'] = "Status changed successfully ..";
					   }
						 if($action=="deactivevate")
					   {
					   $update = $db->query("update ".COUNTRY." set status=0 where id=$val");
					   $_SESSION['suss'] = "Status changed successfully ..";
					   } 
					}
			   }
  ...
  <tr>
    <td align="center" valign="top" class="bg"><table width="970" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
	    ...
	    $sql_select="select * from ".COUNTRY." ".$sql." order by id desc";
    ...
    <thead>
    	<tr>
           ...
           <th width="300px"><a href="javascript:void(0)">Country</a></th>
