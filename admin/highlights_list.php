...
if ($_POST["keyword"] == "del"){
	$id=$_POST['del_id'];
	$select=$db->query("select * from ".HIGH." where id=$id");
    if(mysql_num_rows($select) > 0)
	{
     $delete=$db->query("Delete from ".HIGH." where id=$id");
		if($delete==TRUE)
		{
  ...
  $id=$_POST['del_id']; 
  $update = $db->query("update ".HIGH." set status=0 where id=$id");
  $_SESSION['suss'] = "Status changed successfully ..";
 
 }else if($_POST["keyword"] == "dact"){
  $id=$_POST['del_id'];
  $update = $db->query("update ".HIGH." set status=1 where id=$id");
  $_SESSION['suss'] = "Status changed successfully .."; 
 }
	       ...
	       $action=$_POST['act'];
				...
				{	
					while (list ($key,$val) = each ($box)) 
					{
					   if($action=="delete")
					   {
					    $delete =$db->query("Delete from ".HIGH." where id=$val");
					    $_SESSION['suss'] = "Successfully deleted..";
					   }
						 if($action=="activate")
					   {
					    $update = $db->query("update ".HIGH." set status=1 where id=$val");
					    $_SESSION['suss'] = "Status changed successfully ..";
					   }
						 if($action=="deactivevate")
					   {
					   $update = $db->query("update ".HIGH." set status=0 where id=$val");
					   $_SESSION['suss'] = "Status changed successfully ..";
					   } 
					}
			   }
  ...
  <tr>
    ...
    <thead>
    	<tr>
           ...
           <th width="120px"><a href="javascript:void(0)">Title</a></th>
	...
	?>        
    	<tr>
            ...
            <td><img src="../highlights/thumb/<?php echo $value['banner'];?>" /></td>
          
            <td><?php if($value['status']==1){ ?><a href="javascript: del(<?php echo $value['id']; ?>,'act');" title="Change status to inactive" class="ask vtip"><img src="images/active.jpg" alt="Status" title="" border="0" /></a><?php }else{ ?><a href="javascript: del(<?php echo $value['id']; ?>,'dact');" title="Change status to active" class="ask vtip"><img src="images/inactive.jpg" alt="Status" title="" border="0" /></a><?php } ?>&nbsp;&nbsp;<a href="highlights_edit.php?id=<?php echo $value['id']; ?>" title="Edit Banner" class="vtip"><img src="images/user_edit.png" alt="Edit" title="" border="0" /></a>&nbsp;&nbsp;<a href="javascript: del(<?php echo $value['id']; ?>,'del');" class="ask vtip" title="Delete news"><img src="images/trash.png" alt="Delete" title="" border="0" /></a></td>
        </tr>      
        <?php $m++; } ?>
		 <tfoot>
    	<tr>
